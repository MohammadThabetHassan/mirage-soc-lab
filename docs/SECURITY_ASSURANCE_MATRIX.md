...
| CI supply chain             | Implemented | Workflows use read-only content permissions, avoid privileged PR triggers, publish CodeQL findings and retain SARIF evidence, and keep browser diagnostics only on failure. | Pin third-party actions to verified full SHAs when repository policy and maintenance cadence permit.   |

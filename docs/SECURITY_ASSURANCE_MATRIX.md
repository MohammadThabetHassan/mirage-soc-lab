...
| Error handling              | Partial                 | Procedure-level validation errors and request IDs support safe triage.                                                                   | Add a production error-reporting sink with retention and access controls.                              |
...
| Dependency security         | Implemented             | Production audit is required in CI; dependency remediation policy is documented.                                                         | Enable Dependabot alerts/security updates and dependency review in repository settings.                |
| CI supply chain             | Partial                 | Workflows use read-only content permissions and avoid privileged PR triggers.                                                            | Pin third-party actions to verified full SHAs and add dependency-review workflow.                      |
| Database migration safety   | Partial                 | Reviewed migrations, governance documentation, and atomic write paths exist.                                                             | Add disposable MySQL migration, persistence, and compensating-migration CI tests.                      |
| Transport security          | Deployment prerequisite | Application expects deployment behind HTTPS.                                                                                             | Enforce TLS, HSTS, and secure proxy configuration at the ingress.                                      |

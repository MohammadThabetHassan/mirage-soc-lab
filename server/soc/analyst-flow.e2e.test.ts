...
describe("authenticated analyst journey", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    dbMocks.getSocSnapshot.mockResolvedValue({ cases: [], events: [], runs: [] });
  ...
  it("requires sign-in and supports replay, selection, note disposition, and evaluation", async () => {
    const anonymous = appRouter.createCaller({ ...authenticatedContext(), user: null });
    await expect(anonymous.soc.snapshot()).rejects.toMatchObject({ code: "UNAUTHORIZED" });

    const caller = appRouter.createCaller(authenticatedContext());
    await expect(caller.soc.snapshot()).resolves.toEqual({ cases: [], events: [], runs: [] });

    const replay = await caller.soc.runScenario({ scenarioKey: "full-pipeline" });
    ...
    expect(evaluation.scenarios.map(item => item.classification)).toEqual(["known-positive", "known-positive", "known-benign", "edge-case"]);

...
describe("controlled Cowrie telemetry adapter", () => {
  const fixture = readFileSync(resolve(import.meta.dirname, "fixtures/controlled-cowrie.ndjson"), "utf8");
  ...
  it("imports the redacted fixture deterministically and maps at least five Cowrie event IDs", () => {
    ...
    expect(first.imported.map(event => event.id)).toEqual(second.imported.map(event => event.id));
  ...
  it("rejects malformed, public-source, and invalid records", () => {
    const result = importControlledCowrieJson([
      "not json",
      JSON.stringify({ eventid: "cowrie.login.failed", timestamp: "not-a-date", src_ip: "198.51.100.77" }),
      JSON.stringify({ eventid: "cowrie.login.failed", timestamp: "2026-08-12T09:00:00Z", src_ip: "8.8.8.8" }),
    ].join("\n"));

    expect(result.imported).toHaveLength(0);
    expect(result.rejected.map(item => item.code)).toEqual(["malformed_json", "invalid_record", "invalid_record"]);
  });
});

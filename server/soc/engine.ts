...
export function generateScenario(scenarioKey: ScenarioKey): LabEvent[] {
  if (scenarioKey === "benign-admin") {
    return [
      event(
        ...
        0,
        "auth_success",
        "Authorized admin login accepted.",
        { username: "admin" }
      ),
      event(
        scenarioKey,
  ...
  const failures = Array.from({ length: failureCount }, (_, index) =>
    event(
      ...
      index,
      "auth_failure",
      `SSH authentication failed for account candidate ${index + 1}.`,
      { username: index % 2 ? "root" : "ops" }
    )
  );
  ...
  const success = event(
    ...
    7,
    "auth_success",
    "SSH authentication succeeded after repeated failures.",
    { username: "ops" }
  );
  ...
  return [
    ...
    event(
      ...
      8,
      "decoy_interaction",
      "Session opened with controlled decoy SSH service.",
      { username: "ops" }
    ),
    event(scenarioKey, 9, "discovery", "Discovery-like command observed: id", {
      username: "ops",
      command: "id",
    }),
    event(
      ...
      10,
      "discovery",
      "Discovery-like command observed: cat /etc/passwd",
      { username: "ops", command: "cat /etc/passwd" }
    ),
  ];
}
...
export function detectCases(events: LabEvent[]): DetectedCase[] {
  ...
  if (
    failures.length >=
      (repeatedRule.threshold.minimumFailures ?? Number.MAX_SAFE_INTEGER) &&
    withinMinutes(failures, repeatedRule.correlationWindowMinutes)
  ) {
    detected.push(
      caseFrom(repeatedRule, scenarioKey, failures, {
  ...
  if (
    success &&
    failures.length >=
      (successRule.threshold.minimumFailures ?? Number.MAX_SAFE_INTEGER) &&
    success.occurredAt.getTime() -
      (failures.at(-1)?.occurredAt.getTime() ?? success.occurredAt.getTime()) <=
      successRule.correlationWindowMinutes * 60_000
  ) {
    detected.push(
      caseFrom(successRule, scenarioKey, [...failures, success], {
        failureCount: failures.length,
      })
    );
  }
  ...
  const allSequenceEvents =
    success && decoy ? [...failures, success, decoy, ...discovery] : [];
  if (
    success &&
    decoy &&
    failures.length >=
      (multiStageRule.threshold.minimumFailures ?? Number.MAX_SAFE_INTEGER) &&
    discovery.length >=
      (multiStageRule.threshold.minimumDiscoveryEvents ??
        Number.MAX_SAFE_INTEGER) &&
    multiStageRule.threshold.requiresAuthSuccess !== false &&
    multiStageRule.threshold.requiresDecoyInteraction !== false &&
...
export function evaluateDefinitions() {
  const scenarios = SCENARIOS.map(scenario => {
    ...
    const averageTimeToDetect = detections.length
      ? Math.round(
          (detections.reduce(
            (sum, item) =>
              sum + (item.lastSeenAt.getTime() - firstEvent) / 60_000,
            0
          ) /
            detections.length) *
            10
    ...
    const matchedExpected = scenario.expectedRuleIds.filter(ruleId =>
      observedRuleIds.includes(ruleId)
    ).length;
    return {
      ...scenario,
      ...
      observedRuleIds,
      falsePositives:
        scenario.classification === "known-benign" ? detections.length : 0,
      averageTimeToDetect,
    };
  });
  const expectedTotal = scenarios.reduce(
    (sum, item) => sum + item.expectedDetections,
    0
  );
  const observedTotal = scenarios.reduce(
    (sum, item) => sum + item.observedDetections,
    0
  );
  const falsePositives = scenarios.reduce(
    (sum, item) => sum + item.falsePositives,
    0
  );
  const detectedScenarioCount = scenarios.filter(
    item => item.observedDetections > 0
  ).length;
  ...
  const classMetrics = scenarioClasses.map(classification => {
    const classScenarios = scenarios.filter(
      item => item.classification === classification
    );
    const expected = classScenarios.reduce(
      (sum, item) => sum + item.expectedDetections,
      0
    );
    const observed = classScenarios.reduce(
      (sum, item) => sum + item.observedDetections,
      0
    );
    const matched = classScenarios.reduce(
      (sum, item) => sum + item.matchedExpected,
      0
    );
    const classFalsePositives = classScenarios.reduce(
      (sum, item) => sum + item.falsePositives,
      0
    );
    ...
    return {
      ...
      precision: observed
        ? ((observed - classFalsePositives) / observed) * 100
        : 0,
      recallCoverage: expected ? (matched / expected) * 100 : 0,
      falsePositiveRate: classScenarios.length
        ? (classFalsePositives / classScenarios.length) * 100
        : 0,
      averageTimeToDetect: detected.length
        ? Math.round(
            (detected.reduce((sum, item) => sum + item.averageTimeToDetect, 0) /
              detected.length) *
              10
          ) / 10
        : 0,
  ...
  return {
    ...
    coverage: expectedTotal
      ? (scenarios.reduce((sum, item) => sum + item.matchedExpected, 0) /
        ...
        100
      : 0,
    alertPrecision: observedTotal
      ? ((observedTotal - falsePositives) / observedTotal) * 100
      : 0,
    falsePositiveRate: scenarios.filter(
      item => item.classification === "known-benign"
    ).length
      ? (falsePositives /
          scenarios.filter(item => item.classification === "known-benign")
            .length) *
        100
      : 0,
    averageTimeToDetect: detectedScenarioCount
      ? Math.round(
          (scenarios
            .filter(item => item.observedDetections > 0)
            .reduce((sum, item) => sum + item.averageTimeToDetect, 0) /

...
const PRIVATE_OR_DOCUMENTATION_NETS = [
  "127.",
  "10.",
  "192.168.",
  "172.16.",
...
function safeSourceIp(value: unknown): string | undefined {
  ...
  return ip &&
    PRIVATE_OR_DOCUMENTATION_NETS.some(prefix => ip.startsWith(prefix))
    ? ip
    : undefined;
}
...
function commandIsDiscoveryLike(input: string | undefined): boolean {
  ...
  return /^(id|whoami|hostname|hostnamectl|uname|pwd|ls|cat \/etc\/passwd|netstat|ip addr|ifconfig)\b/.test(
    normalized
  );
}
...
function deterministicEventId(
  ...
  return createHash("sha256")
    .update(`${rawEventId}:${line}:${session ?? "no-session"}`)
    .digest("hex")
    .slice(0, 32);
}

function normalizeRecord(
  record: CowrieRawRecord,
  line: number,
  scenarioKey: string
): NormalizedCowrieEvent | CowrieImportIssue {
  ...
  if (!rawEventId || !timestampText || !sourceIp) {
    return {
      ...
      code: "invalid_record",
      message:
        "Record is missing an allowlisted event ID, valid timestamp, or lab-safe source IP.",
    };
  }
  if (!(rawEventId in COWRIE_EVENT_MAPPINGS)) {
    return {
      ...
      message: `Unsupported Cowrie event ID: ${rawEventId}.`,
    };
  }
  ...
  if (Number.isNaN(occurredAt.getTime())) {
    return {
      line,
      code: "invalid_record",
      message: "Record timestamp is invalid.",
    };
  }
  ...
  const isDiscovery =
    eventType === "discovery" && commandIsDiscoveryLike(input);
  ...
  return {
    ...
    metadata: {
      ...
      raw_session_id: session
        ...
        : "unavailable",
      redaction:
        "passwords, tty logs, transfer paths, hashes, and raw record content excluded",
    },
  };
}
 ...
 */
export function importControlledCowrieJson(
  jsonLines: string,
  scenarioKey = "controlled-cowrie-fixture"
): CowrieImportResult {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    try {
      ...
      rejected.push({
        line,
        code: "malformed_json",
        message: "Line is not valid JSON and was not imported.",
      });
      continue;
    }
    if (!isRecord(parsed)) {
      rejected.push({
        line,
        code: "invalid_record",
        message: "JSON line must contain an object record.",
      });
      continue;
    }
  ...
  return {
    imported,
    rejected,
    acceptedEventIds: Array.from(acceptedEventIds).sort(),
  };
}

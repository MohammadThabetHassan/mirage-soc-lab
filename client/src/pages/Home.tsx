...
export default function Home() {
  ...
  const integrity = trpc.soc.verifyCaseIntegrity.useQuery({ caseId: selectedCase?.id ?? "00000000-0000-0000-0000-000000000000" }, { enabled: Boolean(selectedCase?.id), refetchInterval: 10_000 });
  ...
  const disposition = trpc.soc.dispositionCase.useMutation({
    onSuccess: async () => {
      await Promise.all([utils.soc.snapshot.invalidate(), utils.soc.getCase.invalidate(), utils.soc.verifyCaseIntegrity.invalidate()]);
      ...
      <section className="grid gap-5 2xl:grid-cols-[0.82fr_1.35fr_0.83fr]">
        ...
        <div className="neon-panel min-h-[520px] rounded-xl">
          ...
          {selectedCase ? <div className="space-y-5 p-5">
            ...
            {caseDetail.data?.history.length ? <div className="border-t border-cyan-400/10 pt-4"><h3 className="neon-label mb-3 text-[10px] text-fuchsia-300">Immutable disposition history</h3><div className="space-y-2">{caseDetail.data.history.map(entry => <div key={entry.id} className="rounded-lg border border-fuchsia-400/15 bg-fuchsia-400/5 p-3"><div className="flex justify-between gap-3 text-[10px]"><span className="font-medium uppercase text-fuchsia-200">{entry.disposition}</span><span className="font-mono text-slate-500">{formatTime(entry.createdAt)}</span></div><p className="mt-1 text-xs text-slate-300">{entry.authorName}</p><p className="mt-1 text-xs leading-5 text-slate-400">{entry.note}</p><p className="mt-2 font-mono text-[9px] text-slate-500">CHAIN {entry.entryHash.slice(0, 12)}…</p></div>)}</div></div> : <div className="border-t border-cyan-400/10 pt-4 text-xs text-slate-500">No disposition has been recorded. Future decisions are appended to this immutable history.</div>}
            <div className={`border-t border-cyan-400/10 pt-4 ${integrity.data?.verified ? "" : ""}`}><h3 className="neon-label mb-2 text-[10px] text-cyan-300">Integrity verification</h3>{integrity.isLoading ? <p className="text-xs text-slate-500">Recomputing append-only chains…</p> : integrity.data?.verified ? <div className="rounded-lg border border-emerald-400/20 bg-emerald-400/5 p-3"><p className="text-xs font-semibold text-emerald-200">Verified evidence and disposition chains</p><p className="mt-1 text-[11px] text-slate-400">{integrity.data.evidence.entries} evidence links and {integrity.data.dispositions.entries} analyst decisions recomputed successfully.</p></div> : <div className="rounded-lg border border-rose-400/25 bg-rose-400/5 p-3"><p className="text-xs font-semibold text-rose-200">Integrity verification unavailable or failed</p><p className="mt-1 text-[11px] text-slate-400">Review the evidence lineage and disposition history before relying on this case.</p></div>}</div>
            {caseDetail.data?.evidenceLineage.length ? <div className="border-t border-cyan-400/10 pt-4"><h3 className="neon-label mb-3 text-[10px] text-cyan-300">Evidence lineage</h3><p className="mb-2 text-xs text-slate-500">Rule version {caseDetail.data.ruleVersion}; each event link extends the case hash chain.</p><div className="space-y-1">{caseDetail.data.evidenceLineage.map(entry => <div key={entry.id} className="flex items-center justify-between rounded bg-slate-950/35 px-3 py-2 font-mono text-[10px]"><span className="text-cyan-100">{entry.eventId}</span><span className="text-slate-500">{entry.entryHash.slice(0, 12)}…</span></div>)}</div></div> : null}
